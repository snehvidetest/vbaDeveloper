...
Private Function Testcase(tc As Integer)
    ...
    Select Case parameters("testSubject")
        Case "nextStep"
            frm044.CommandButton2_Click 'Click on Videre button
            result = Global_Test_Func.NextStep(parameters("expected"))
            
        Case "backButton"
            frm044.CommandButton1_Click
        ...
        Case "checkCaption"
            If (parameters("testParameter") = "buttonOne") Then
                result = frm044.CommandButton1.Caption
            ElseIf (parameters("testParameter") = "buttonTwo") Then
                result = frm044.CommandButton2.Caption
            ElseIf (parameters("testParameter") = "beskrivelse") Then
                result = frm044.Label1.Caption
            End If
...
Private Function KillForms()
    'Closes forms
    ThisWorkbook.Activate
    If Global_Test_Func.IsLoaded("frm044") Then
        Unload frm044
    End If
    If Global_Test_Func.IsLoaded("frm035") Then
        Unload frm035
    End If
